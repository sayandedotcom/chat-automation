...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!tokenResponse.ok) {
      ...
      console.error("Google Slides token exchange failed:", errorData);
      return NextResponse.redirect(
    ...
    cookieStore.set("google_slides_access_token", tokens.access_token, {
      httpOnly: true,
    ...
    if (tokens.refresh_token) {
      cookieStore.set("google_slides_refresh_token", tokens.refresh_token, {
        httpOnly: true,
        secure: process.env.NODE_ENV === "production",
        sameSite: "lax",
    ...
    try {
      ...
      if (syncResponse.ok) {
        console.log("✅ Google Slides credentials synced to MCP");
      } else {
        console.error(
    ...
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_APP_URL}/integrations?success=google-slides`,
    );
  } catch (error) {
    console.error("Google Slides OAuth error:", error);
    return NextResponse.redirect(
