...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    cookieStore.set("google_docs_access_token", tokens.access_token, {
      httpOnly: true,
    ...
    if (tokens.refresh_token) {
      cookieStore.set("google_docs_refresh_token", tokens.refresh_token, {
        httpOnly: true,
        secure: process.env.NODE_ENV === "production",
        sameSite: "lax",
    ...
    try {
      ...
      if (syncResponse.ok) {
        console.log("✅ Google Docs credentials synced to MCP");
      } else {
        console.error(
    ...
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_APP_URL}/integrations?success=google-docs`
    );
  } catch (error) {
