...
The system supports Gmail, Google Docs, Sheets, Slides, Calendar, Drive, Notion, Slack, and web search — all pluggable via a YAML config.
...
The agent supports 50+ MCP tools across 10+ integrations. Feeding all of them into every LLM call wastes tokens and confuses the model — tool-calling accuracy drops when the LLM has to choose among 50 irrelevant options.
...
1. **Classify** — An LLM classifier (Gemini Flash, `temperature=0`, structured output) takes the user request and the list of available integrations, and returns the minimum set needed. The classifier uses `with_structured_output(ClassifierOutput)` — no manual JSON parsing, no edge cases around markdown code fences.
...
4. **Bind tools** — Filter the full tool registry to just the classified integrations, bind them to the executor LLM, and create a fresh `ToolNode`.

**Why this matters:** This is the single biggest decision for production quality. Without it, every request would dump 50+ tool schemas into the context window. With it, a "send an email" request gets 5-8 Gmail tools, not the full 50+. Token cost drops, accuracy improves, and latency decreases.

**The safety net:** If the classifier under-classifies and the executor tries to call a tool from an unloaded integration, the executor catches the tool-not-found error, loads the missing integration on-the-fly, and retries. The user never sees the error.
...
The executor invokes the LLM, which generates tool calls. The graph routes to the `tools` node, which executes them. Then `route_after_tools` sends the results **back to the executor**. The executor sees the tool results, reasons about them, and either generates more tool calls or finishes. This loop continues until the LLM produces a final text response (no more tool calls) or the safety cap of 10 tool calls per step is hit.
...
**Phase 1: PREVIEW** — The executor runs the LLM to generate tool calls, but **doesn't execute them.** Instead, it:
...
**Phase 2: USER DECISION** — The frontend renders the approval preview as an editable form. The user can:
...
1. **Marks the current step as completed** with its result text
2. **Extracts structured artifacts** — scans tool message responses for document URLs, email IDs, spreadsheet IDs, event links. Uses a two-phase approach: first tries JSON field matching (e.g., `documentId`, `spreadsheetId`), then falls back to regex URL extraction.
3. **Extracts search results** — if the step involved web search, parses Tavily's response into structured `SearchResultItem` objects (title, URL, domain, favicon).
4. **Advances `current_step_index`** — moves to the next step.
5. **Resets executor state** — clears `_executor_chat` and `_step_tool_calls` so the next step starts fresh.
...
Adding a new integration requires no Python changes. Add a YAML entry with `tool_names`, `mcp_server`, keywords, and hints. The registry picks it up at startup, the classifier indexes it, the smart router can classify it, and the executor can use its tools. This was a deliberate architectural choice — integration knowledge lives in config, not in code.

### Supported Integrations

| Integration         | Capabilities                            |
| ------------------- | --------------------------------------- |
| Gmail               | Send, draft, search, read, label emails |
| Google Docs         | Create, edit, format documents          |
| Google Sheets       | Create, read, write spreadsheets        |
| Google Slides       | Create, edit presentations              |
| Google Calendar     | Create, list, update events             |
| Google Drive        | Search, manage, share files             |
| Notion              | Create pages, search workspace          |
| Web Search (Tavily) | Search, extract, research web content   |
...
**Phase 1: Structured matching** — Each integration has known ID fields (`documentId` for Docs, `spreadsheetId` for Sheets, `messageId` for Gmail). The extractor tries all integration configs against every tool response. Unique fields match unambiguously; generic `id` fields require a confirming URL field to disambiguate.

**Phase 2: Regex fallback** — If structured matching finds nothing, regex patterns extract URLs from tool responses and classify them by domain (`docs.google.com/document` -> document, `notion.so` -> page).
